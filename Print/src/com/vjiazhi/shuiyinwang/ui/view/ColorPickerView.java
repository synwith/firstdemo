...
public class ColorPickerView extends View {
	...
	public void init() {
		int width = (int) (MyConfig.getScreenWidth() * 0.8);
		...
		r = width / 3 * 0.7f - mPaint.getStrokeWidth() * 0.5f;
		...
		rectLeft = r + mPaint.getStrokeWidth() * 0.5f + 65;
		rectTop = -r;
		rectRight = r + mPaint.getStrokeWidth() * 0.5f + 115;
		rectBottom = r;

	}

	public ColorPickerView(Context context, int height, int width, int initColor) {
		super(context);
		init();
	}
	...
	protected void onDraw(Canvas canvas) {
		// 移动中心
		canvas.translate(mWidth / 3, mHeight / 2);
		...
		L.l("=====R:" + r);
		L.l("====left:" + rectLeft + "==top:" + rectTop + "===right:"
				+ rectRight + "==bottom:" + rectBottom);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			L.l("==============hightCenter:"+highlightCenter+"====inCenter:"+inCenter+"===color:"+mCenterPaint.getColor());
			if (highlightCenter && inCenter) {// 点击在中心圆, 且当前启动在中心圆
				if (myOnClickListener != null) {
					myOnClickListener.colorChanged(mCenterPaint.getColor());
				}
			}
			if (downInCircle) {
				downInCircle = false;
