...
//	}
	...
	public static Bitmap createFinalBitmapByBitmap(Bitmap srcBitmap, Bitmap shuiyinBitmap, WaterMarkPosition waterMarkPosition){
		...
		if (waterMarkPosition != null) {
			matrix = convertToRelativeMatrix(outBitmap, waterMarkPosition);
		}
	 ...
	 * @author fengyi add
	 */
	public static int calculateScaleRate(int width, int height){
		int i = 0;
		while(true){
			if ((width >> i <= 1000)
					&& (height >> i <= 1000)) {
...
//		if((matrixTranslateX + waterMarkPosition.getWaterMarkWidth())> width){
//			matrixTranslateX = width - waterMarkPosition.getWaterMarkWidth();
//		}
//		
//		if((matrixTranslateY + waterMarkPosition.getWaterMarkHeight())> height){
//			matrixTranslateY = height - waterMarkPosition.getWaterMarkHeight();
//		}
